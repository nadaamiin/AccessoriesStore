function OrderSummaryDetails({
  ...
  return (
    <div className="space-y-4">
        ...
        <div key={item.productId} className="flex items-center gap-3">
          ...
          {item.isOnSale ? (
            <div className="text-right leading-tight">
              ...
              <p className="text-espresso font-semibold">LE {(item.price * item.quantity).toFixed(2)}</p>
            </div>
          ) : (
            <p className="text-espresso font-semibold">LE {(item.price * item.quantity).toFixed(2)}</p>
          )}
      ...
      <span className="font-body font-semibold text-base text-espresso">Total</span>
      <span className="font-body font-bold text-xl text-espresso">LE {total.toFixed(2)}</span>
    </div>

      {totalSavings > 0 && (
        <p className="text-xs text-muted">Total savings LE {totalSavings.toFixed(2)}</p>
      )}
    </div>
  );
}
