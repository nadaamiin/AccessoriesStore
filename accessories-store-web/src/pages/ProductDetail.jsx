      ...
      <div className="max-w-6xl mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="flex gap-3">
          {images.length > 1 && (
            <div className="flex flex-col gap-2">
              {images.map((img, i) => (
                ...
                >
                  <img src={`${import.meta.env.VITE_SERVER_URL}${img}`} alt="" className="w-full h-full object-cover" />
                </button>
              ))}
            </div>
          )}
          <div className="relative flex-1 aspect-square rounded-2xl overflow-hidden bg-blush-100">
            {images.length > 0 ? (
              <img
                src={`${import.meta.env.VITE_SERVER_URL}${images[activeImage]}`}
                alt={product.name}
        ...
        <div>
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button
              onClick={handleFav}
              className="w-12 h-12 flex items-center justify-center hover:scale-110 transition-transform shrink-0"
            ...
            >
              <svg
                ...
                height="25"
                viewBox="0 0 24 24"
                fill={isFavorite(product.id) ? "#dd9cc1" : "none"}
                stroke="#dd9cc1"
                strokeWidth="1.8"
              >
                <path
