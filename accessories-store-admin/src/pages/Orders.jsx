...
function OrderCard({ order, onStatusChange, updating }) {  
  return (  
    <div className="bg-white border border-nude-200 rounded-lg p-5">  
...
function Orders() {  
  ...
  const visibleOrders = filteredOrders.filter(  
    (o) => statusTab === "All" || o.status === statusTab  
  );  
  ...
  const content = loading ? (  
    ...
    <div className="p-6 md:p-10">  
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">  
        ...
        <div className="inline-flex flex-wrap bg-nude-100 rounded-md p-1 self-start">  
          {STATUS_TABS.map((s) => (  
            <button  
              key={s}  
              onClick={() => setStatusTab(s)}  
              className={`px-4 py-1.5 rounded text-sm font-medium transition ${  
                statusTab === s ? "bg-white text-espresso shadow-sm" : "text-muted hover:text-espresso"  
              }`}  
            >  
              {s}  
            </button>  
      ...
      <h2 className="text-sm font-medium tracking-wide uppercase text-muted mb-3">  
        {statusTab === "All" ? "All Orders" : statusTab} ({visibleOrders.length})  
      </h2>  
